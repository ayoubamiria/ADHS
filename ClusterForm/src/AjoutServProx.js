...
function AjoutServProx() {
  ...
  const getTemplates = async (server) => {
    try {
      const response = await fetch("http://localhost:5000/connect_and_get_templates", {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        const updatedServers = servers.map((s) =>
          s.id === server.id ? { ...s, templates: data.templates } : s
        );
        setServers(updatedServers);
        localStorage.setItem("servers", JSON.stringify(updatedServers));
      } else {
        alert(`Échec de la récupération des templates : ${data.error}`);
      }
    } catch (error) {
