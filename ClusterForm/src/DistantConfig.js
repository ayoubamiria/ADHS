  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!isSameServer) {
        ...
        if (!data.success) {
          alert(`Erreur lors du clonage : ${data.message}`);
          return;
        }
        alert("Clonage réussi !");
      }
  
      const createResponse = await fetch("http://localhost:5000/create_vmprox", {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (createData.error) {
        alert(`Erreur lors de la création de la VM : ${createData.error}`);
      } else {
